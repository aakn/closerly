				...
				<div class="span12">
					<section id="registered">
						<div class="page-header">
							<h2>List of People who have registered</h2>
						</div>
							...
							<tbody>
								<?php
									...
									while ($row = mysql_fetch_array($result)) {
										$name = $row['name'];
										$email = $row['email'];
										echo "<tr><td>$i</td><td>$name</td><td>$email</td></tr>";
										$i++;
									}
								?>
					...
					<section id="wait">
						<div class="page-header">
							<h2>List of People who are in the wait-list</h2>
						</div>
						<table class="table table-bordered table-striped table-hover">
							<thead>
								<tr>
							...
							<tbody>
								<?php
									...
									include('../config.inc');
									$query = "SELECT email,code FROM wait_list";
									$result = mysql_query($query);
									$i = 1;
									while ($row = mysql_fetch_array($result)) {
										$code = $row['code'];
										$email = $row['email'];
										echo "<tr><td>$i</td><td>$email</td><td>$code</td></tr>";
										$i++;
									}
									if($i == 1) {
										echo "<tr><td>0</td><td>Empty</td><td>Empty</td></tr>";
									}
								?>
					...
					<section id="invited">
						<div class="page-header">
							<h2>List of People who have been invited by us</h2>
						</div>
						<table class="table table-bordered table-striped table-hover">
							<thead>
								<tr>
							...
							<tbody>
								<?php
									...
									include('../config.inc');
									$query = "SELECT name,email FROM invited";
									$result = mysql_query($query);
									$i = 1;
									while ($row = mysql_fetch_array($result)) {
										$name = $row['name'];
